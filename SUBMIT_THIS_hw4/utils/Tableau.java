...
public class Tableau {
   ...
   */
        private boolean checkForEnemyHit(DefenderProjectile d) {
        	...
        	for(int i = 0; i < enemyFleet.length; i++)
        		for(int k = 0; k < enemyFleet[i].length; k++)
        			if(enemyFleet[i][k] == null){
        				//do nothing
        			}
        			else if(d.hit(enemyFleet[i][k].getBounds())) {
        				enemyFleet[i][k].takeHit();
        				if(enemyFleet[i][k].getArmor() <= 0) {
        					score += enemyFleet[i][k].getPoints();
        					enemyFleet[i][k] = null;
        				}
        				return true;
        			}
   ...
   */
        private SpaceShip getRightMostEnemy() {
        	InvaderShip rightMostEnemy = enemyFleet[enemyFleet.length - 1][enemyFleet[0].length - 1];
        	
        	for(int i = enemyFleet.length - 1; i >= 0; i--) {
        		for(int k = enemyFleet[i].length - 1; k >= 0; k--) {
        			if(enemyFleet[k][i] != null) {
        				return enemyFleet[k][i];
        			}
        		}	
        	}
        	return null;          
